      ...
      <div className="grid grid-cols-2 gap-3">
        {isLoading && products.length === 0 ? (
          Array.from({ length: 4 }).map((_, i) => (
            <div key={`skeleton-${i}`} className="p-3 rounded-3xl bg-secondary/20 border border-white/5 flex flex-col justify-between h-full relative overflow-hidden animate-pulse">
              <div className="w-full aspect-square rounded-2xl bg-secondary/30 mb-3" />
              <div className="h-3 w-16 bg-zinc-800/50 rounded mb-1" />
              <div className="h-4 w-3/4 bg-zinc-800/50 rounded mb-1" />
              <div className="h-5 w-1/2 bg-zinc-800/50 rounded mb-3" />
              <div className="flex gap-1 border-t border-white/5 pt-3">
                <div className="h-8 flex-1 bg-zinc-800/50 rounded-xl" />
                <div className="h-8 w-8 shrink-0 bg-zinc-800/50 rounded-xl" />
              </div>
            </div>
          ))
          ...
          <div className="col-span-2 py-8">
            <EmptyState 
              ...
              description={busca ? "Não encontramos produtos com esse nome." : "Cadastre produtos para vender na sua barbearia."}
              actionLabel={busca ? undefined : "Adicionar Produto"}
